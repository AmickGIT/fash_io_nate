...
from networks import (
    ConditionGenerator, VGGLoss, GANLoss, load_checkpoint, save_checkpoint,
    define_D, make_grid as mkgrid, make_grid_3d as mkgrid_3d
)
from utils import *
...
def freeze_tocg_final_layers(tocg, num_unfrozen=2):
    """
    Freezes all layers in tocg except for the last num_unfrozen (as ordered in .named_children()).
    """
    layers = list(tocg.named_children())
    ...
    unfrozen_layer_names = [layers[idx][0] for idx in range(total_layers - num_unfrozen, total_layers)]
    print("Unfrozen ConditionGenerator layers for finetuning:", unfrozen_layer_names)
...
def get_opt():
    ...
    parser.add_argument('--Ddownx2', action='store_true', help="Downsample D's input")
    ...
    # Custom: number of last layers to unfreeze for finetuning
    parser.add_argument('--unfreeze_last', type=int, default=2, help="Number of last condition generator layers to finetune")
...
def train(opt, train_loader, tocg, D):
    ...
    # Only update unfrozen params:
    optimizer_G = torch.optim.Adam(
        filter(lambda p: p.requires_grad, tocg.parameters()),
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        warped_clothmask_paired_taco_onehot = torch.FloatTensor((warped_clothmask_paired_taco.detach().cpu().numpy() > 0.5).astype(np.float64)).cuda()
...
def main():
    ...
    # Freeze all but last N layers of ConditionGenerator for limited finetune
    freeze_tocg_final_layers(tocg, num_unfrozen=opt.unfreeze_last)
